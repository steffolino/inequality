...
// .wrangler/tmp/bundle-gaEkCM/strip-cf-connecting-ip-header.js
  ...
  async fetch(req, env) {
    ...
    try {
      if (pathname === "/api/series" && req.method === "GET") {
        ...
        const region = (searchParams.get("r") || searchParams.get("region") || "DE").toUpperCase();
        ...
        if (indicator === "gini") {
          ...
          return allowCORS(req, ok({ indicator, group, region, scenario, from, to, data: results }));
        }
        return allowCORS(req, ok({ indicator, group, region, scenario, from, to, data: [] }));
      }
...
// .wrangler/tmp/bundle-gaEkCM/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-gaEkCM/middleware-loader.entry.ts
